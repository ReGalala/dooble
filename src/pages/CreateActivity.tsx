...
const CreateActivity = () => {
  ...
  const [lat, setLat] = useState("57.7089");
  const [lng, setLng] = useState("11.9746");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await create({
      ...
      lat: parseFloat(lat) || 57.7089,
      lng: parseFloat(lng) || 11.9746,
    ...
    toast({ title: "Activity created", description: `"${name}" is now live on the map.` });
    navigate("/dashboard");
  };
          ...
          <div className="rounded-lg border border-border bg-white shadow-card p-6 md:p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <Field label="Description *">
                <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe your activity in 2–4 sentences…" rows={4} className="bg-secondary/10" />
              </Field>

              <Field label="Address *">
                <Input value={address} onChange={(e) => setAddress(e.target.value)} onBlur={handleAddressBlur} placeholder="e.g. Stenpiren, Gothenburg" />
                {geocoding && <p className="text-xs text-muted-foreground">Looking up coordinates…</p>}
              </Field>

              <div className="grid grid-cols-2 gap-4">
                <Field label="Latitude (auto-filled)">
                  <Input value={lat} onChange={(e) => setLat(e.target.value)} placeholder="57.7089" className="bg-muted/50" />
                </Field>
                <Field label="Longitude (auto-filled)">
                  <Input value={lng} onChange={(e) => setLng(e.target.value)} placeholder="11.9746" className="bg-muted/50" />
                </Field>
              </div>

              <Field label="Opening hours (optional)">
                <Input value={time} onChange={(e) => setTime(e.target.value)} placeholder="e.g. 09:00–17:00" />
              </Field>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Price (SEK)">
                  <Input type="number" min="0" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="0 = Free" />
                </Field>
                <Field label="Number of tickets *">
                  <Input type="number" min="1" value={tickets} onChange={(e) => setTickets(e.target.value)} placeholder="e.g. 50" />
                </Field>
              </div>
